...
<?php
    ...
    // $sql = "SELECT b.myBoardID, b.boardTitle, b.boardContents, m.youName, b.regTime, b.boardView FROM myBoard b JOIN myMember m ON(b.myMemberID = m.myMemberID) WHERE b.youName LIKE '%{$searchKeyword}%' ORDER BY myBoardID DESC LIMIT 10";
...
<?php
    ...
    if($page != 1){
        $prevPage = $page - 1;
        echo "<li><a href='board.php?searchKeyword={$searchKeyword}&searchOption={$searchOption}&page=1'>처음으로</a></li>";
        echo "<li><a href='board.php?searchKeyword={$searchKeyword}&searchOption={$searchOption}&page={$prevPage}'>이전</a></li>";
    }
    ...
    for($i=$startPage; $i<=$endPage; $i++){
        ...
        echo "<li class='{$active}'><a href='board.php?searchKeyword={$searchKeyword}&searchOption={$searchOption}&page={$i}'>{$i}</a></li>";
    }

    // 다음 페이지, 마지막 페이지
    if($page != $boardCount){
        $nextPage = $page + 1;
        echo "<li><a href='board.php?searchKeyword={$searchKeyword}&searchOption={$searchOption}&page={$nextPage}'>다음</a></li>";
        echo "<li><a href='board.php?searchKeyword={$searchKeyword}&searchOption={$searchOption}&page={$boardCount}'>마지막으로</a></li>";
    }
?>
                    </ul>
                </div>
